    ...
    <div class="content-wrapper">
        ...
        <div class="content-header row">
            <div class="content-header-left col-md-4 col-12 mb-2">
                <h2 class="content-header-title">ISBAT</h>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                ...
                                <div class="text-left">
                                    <a href="javascript::" id="btnAdd" class="btn btn-icon btn-success mb-0" data-toggle="modal" data-target="#modalIsbat"><i class="ft-user-plus"></i> &nbsp;Tambah Pasutri</a>
                                </div>
                            ...
                            <div class="card-content collapse show">
                                <div class="card-body">
                                    ...
                                    <table class="table table-striped table-responsive" id="dataTableIsbat">
                                        <thead style="word-wrap: break-word; word-break: break-all; white-space: normal;">
                                            <tr>
                                                ...
                                                <th scope="col" style="width: 5%;">Jadwal</th>
                                                <th scope="col" style="width: 50%">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody id="showDataIsbat">
...
<!-- MODAL VIEW BERKAS -->
<div class="modal fade" id="modalIsbat" role="dialog" aria-labelledby="modalIsbatLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalIsbatLabel"><strong>TAMBAH PASUTRI</strong></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= BASE_URL . 'staff/submitRegistration/IsbatByOfficer'; ?>" method="post" id="formAddEditIsbat">
                    <!-- DATA HEADER ISBAT -->
                    <div class="form-group row">
                        <div class="col-md-12">
                            <div class="card text-white box-shadow-0 bg-success">
                                <div class="card-content collapse show">
                                    <div class="card-body">
                                        <p class="card-text"><i class="ft-users"></i>&nbsp; DATA HEADER ISBAT</strong></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            <div class="card text-white box-shadow-0 bg-success">
                                <div class="card-content collapse show">
                                    <div class="card-body">
                                        <p class="card-text"><i class="icon-symbol-male"></i>&nbsp; DATA SUAMI</strong></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-2">
                            <input type="button" class="btn btn-info btn-min-width" value="Cek NIK" id="isb_btnceknik_s" style="margin-left: -20px;">
                        </div>
                    </div>
                    <div class="form-group row">
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            <div class="card text-white box-shadow-0 bg-success">
                                <div class="card-content collapse show">
                                    <div class="card-body">
                                        <p class="card-text"><i class="icon-symbol-female"></i>&nbsp; DATA ISTRI</strong></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-2">
                            <input type="button" class="btn btn-info btn-min-width" value="Cek NIK" id="isb_btnceknik_i" style="margin-left: -20px;">
                        </div>
                    </div>
                    <div class="form-group row">
...
<script>
    ...
    $(function() {
        ...
        $('#btnSave').click(function() {
            var url = $('#formAddEditIsbat').attr('action');
            var data = $('#formAddEditIsbat').serialize();
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: url,
                ...
                success: function(response) {
                    if (response.success) {
                        $('#modalIsbat').modal('hide');
                        $('#formAddEditIsbat')[0].reset();
                        if (response.type == 'add') {
                            type = 'ditambahkan'
                        } else if (response.type == 'update') {
                            type = 'diubah'
                        }
                        swal("Selamat!", "Data pasutri berhasil " + type + "!", "success");
                        showDataIsbat();
                    } else {
                ...
                error: function() {
                    swal("Error!", "Gagal menambahkan data pasutri!", "error");
                }
            });
        });
        ...
        $('#btnAdd').click(function() {
            $('#isb_btnceknik_s').show();
            $('#isb_btnceknik_i').show();
            //header
            // $('select[name=isb_tempat]').val('0');
            // $('select[name=isb_tempat]').removeAttr('disabled');
            $('input[name=isb_tanggal_akad]').removeAttr('readonly');
            $('input[name=isb_jam_akad]').removeAttr('readonly');
            $('input[name=isb_alamat_akad]').removeAttr('readonly');

            //suami
            $('select[name=isb_kewarganegaraan_s]').val('0');
            $('select[name=isb_kewarganegaraan_s]').removeAttr('readonly');
            $('input[name=isb_nik_s]').removeAttr('readonly');
            $('input[name=isb_nama_s]').removeAttr('readonly');
            $('input[name=isb_ttl_s]').removeAttr('readonly');
            $('input[name=isb_ttl2_s]').removeAttr('readonly');
            $('input[name=isb_umur_s]').removeAttr('readonly');
            // $('select[name=isb_status_s]').val('0');
            // $('select[name=isb_status_s]').removeAttr('readonly');
            $('input[name=isb_agama_s]').val('');
            $('input[name=isb_agama_s]').removeAttr('readonly');
            $('input[name=isb_alamat_s]').removeAttr('readonly');
            $('input[name=isb_pekerjaan_s]').val(null).trigger('change');
            $('input[name=isb_pekerjaan_s]').removeAttr('readonly');
            $('input[name=isb_nohp_s]').removeAttr('readonly');

            //istri
            $('select[name=isb_kewarganegaraan_i]').val('0');
            $('select[name=isb_kewarganegaraan_i]').removeAttr('readonly');
            $('input[name=isb_nik_i]').removeAttr('readonly');
            $('input[name=isb_nama_i]').removeAttr('readonly');
            $('input[name=isb_ttl_i]').removeAttr('readonly');
            $('input[name=isb_ttl2_i]').removeAttr('readonly');
            $('input[name=isb_umur_i]').removeAttr('readonly');
            // $('select[name=isb_status_i]').val('0');
            // $('select[name=isb_status_i]').removeAttr('readonly');
            $('input[name=isb_agama_i]').val('');
            $('input[name=isb_agama_i]').removeAttr('readonly');
            $('input[name=isb_alamat_i]').removeAttr('readonly');
            $('input[name=isb_pekerjaan_i]').val(null).trigger('change');
            $('input[name=isb_pekerjaan_i]').removeAttr('readonly');
            $('input[name=isb_nohp_i]').removeAttr('readonly');
            $('#btnSave').show();
            $('#modalIsbat').find('.modal-title').text('TAMBAH PASUTRI');
            $('#formAddEditIsbat').attr('action', '<?= BASE_URL . 'staff/submitRegistration/IsbatByOfficer^add^0'; ?>');
            $('#formAddEditIsbat')[0].reset();
        });

        //EDIT DATA ISBAT
        $('#showDataIsbat').on('click', '.editDataIsbat', function() {
            var regId = $(this).attr('data');
            $('#btnSave').show();
            $('#isb_btnceknik_s').show();
            $('#isb_btnceknik_i').show();
            $('#modalIsbat').modal('show');
            $('#modalIsbat').find('.modal-title').text('UBAH DATA PASUTRI');
            $('#formAddEditIsbat').attr('action', '<?= BASE_URL . 'staff/submitRegistration/IsbatByOfficer^update^' ?>' + regId);
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= BASE_URL . 'staff/getDetailIsbat'; ?>',
                data: {
                    regId: regId
                },
                async: false,
                ...
                success: function(data) {
                    for (i = 0; i < data.length; i++) {
                        //header
                        // $('select[name=isb_tempat]').attr('disabled', true);
                        $('input[name=isb_tanggal_akad]').removeAttr('readonly');
                        $('input[name=isb_jam_akad]').removeAttr('readonly');
                        $('input[name=isb_alamat_akad]').removeAttr('readonly');
                        ...
                        //header
                        // $('select[name=isb_tempat]').val(data[i].TMP_NIKAH);
                        // $('#isb_tempat_edit').val(data[i].TMP_NIKAH);
                        var objTgl = data[i].TGL_AKAD;
                        var tgl = objTgl.split(' ');
                        $('input[name=isb_tanggal_akad]').val(tgl[0]);
                        $('input[name=isb_jam_akad]').val(tgl[1]);
                        $('input[name=isb_alamat_akad]').val(data[i].ALMT_AKAD);

                        //suami
                        $('select[name=isb_kewarganegaraan_s]').val(data[i].KEWARGANEGARAAN_S);
                        $('input[name=isb_nik_s]').val(data[i].NIK_S);
                        $('input[name=isb_nama_s]').val(data[i].NAMA_S);
                        var objTtl = data[i].TTL_S;
                        var ttl = objTtl.split(' ');
                        $('input[name=isb_ttl_s]').val(ttl[0].substr(0, ttl[0].length - 1));
                        $('input[name=isb_ttl2_s]').val(ttl[1]);
                        $('input[name=isb_umur_s]').val(data[i].UMUR_S);
                        // $('select[name=isb_status_s]').val(data[i].STATUS_S);
                        $('input[name=isb_agama_s]').val(data[i].AGAMA_S);
                        $('input[name=isb_alamat_s]').val(data[i].ALMT_S);
                        $('input[name=isb_pekerjaan_s]').val(data[i].PEKERJAAN_S).trigger('change');
                        $('input[name=isb_nohp_s]').val(data[i].NO_HP_S);

                        //istri
                        $('select[name=isb_kewarganegaraan_i]').val(data[i].KEWARGANEGARAAN_I);
                        $('input[name=isb_nik_i]').val(data[i].NIK_I);
                        $('input[name=isb_nama_i]').val(data[i].NAMA_I);
                        var objTtl = data[i].TTL_I;
                        var ttl = objTtl.split(' ');
                        $('input[name=isb_ttl_i]').val(ttl[0].substr(0, ttl[0].length - 1));
                        $('input[name=isb_ttl2_i]').val(ttl[1]);
                        $('input[name=isb_umur_i]').val(data[i].UMUR_I);
                        // $('select[name=isb_status_i]').val(data[i].STATUS_I);
                        $('input[name=isb_agama_i]').val(data[i].AGAMA_I);
                        $('input[name=isb_alamat_i]').val(data[i].ALMT_I);
                        $('input[name=isb_pekerjaan_i]').val(data[i].PEKERJAAN_I).trigger('change');
                        $('input[name=isb_nohp_i]').val(data[i].NO_HP_I);
                    }
                },
                error: function() {
                    swal("Error!", "Internal Server error 500!", "error");
        ...
        //VIEW ISBAT
        $('#showDataIsbat').on('click', '.viewDetailIsbat', function() {
            var regId = $(this).attr('data');
            $('#isb_btnceknik_s').hide();
            $('#isb_btnceknik_i').hide();
            $('#modalIsbat').modal('show');
            $('#modalIsbat').find('.modal-title').text('LIHAT DATA ISBAT');
            $('#btnSave').hide();
            $.ajax({
                type: 'ajax',
                method: 'post',
                ...
                success: function(data) {
                    for (i = 0; i < data.length; i++) {
                        ...
                        var objTgl = data[i].TGL_AKAD;
                        var tgl = objTgl.split(' ');
                        $('input[name=isb_tanggal_akad]').attr('readonly', true);
                        $('input[name=isb_tanggal_akad]').val(tgl[0]);
                        $('input[name=isb_jam_akad]').attr('readonly', true);
                        $('input[name=isb_jam_akad]').val(tgl[1]);
                        $('input[name=isb_alamat_akad]').attr('readonly', true);
                        $('input[name=isb_alamat_akad]').val(data[i].ALMT_AKAD);

                        //suami
                        $('select[name=isb_kewarganegaraan_s]').attr('readonly', true);
                        $('select[name=isb_kewarganegaraan_s]').val(data[i].KEWARGANEGARAAN_S);
                        $('input[name=isb_nik_s]').attr('readonly', true);
                        $('input[name=isb_nik_s]').val(data[i].NIK_S);
                        $('input[name=isb_nama_s]').attr('readonly', true);
                        $('input[name=isb_nama_s]').val(data[i].NAMA_S);
                        var objTtl = data[i].TTL_S;
                        var ttl = objTtl.split(' ');
                        $('input[name=isb_ttl_s]').attr('readonly', true);
                        $('input[name=isb_ttl_s]').val(ttl[0].substr(0, ttl[0].length - 1));
                        $('input[name=isb_ttl2_s]').attr('readonly', true);
                        $('input[name=isb_ttl2_s]').val(ttl[1]);
                        $('input[name=isb_umur_s]').attr('readonly', true);
                        $('input[name=isb_umur_s]').val(data[i].UMUR_S);
                        // $('select[name=isb_status_s]').attr('readonly', true);
                        // $('select[name=isb_status_s]').val(data[i].STATUS_S);
                        $('input[name=isb_agama_s]').attr('readonly', true);
                        $('input[name=isb_agama_s]').val(data[i].AGAMA_S);
                        $('input[name=isb_alamat_s]').attr('readonly', true);
                        $('input[name=isb_alamat_s]').val(data[i].ALMT_S);
                        $('input[name=isb_pekerjaan_s]').attr('readonly', true);
                        $('input[name=isb_pekerjaan_s]').val(data[i].PEKERJAAN_S).trigger('change');
                        $('input[name=isb_nohp_s]').attr('readonly', true);
                        $('input[name=isb_nohp_s]').val(data[i].NO_HP_S);

                        //istri
                        $('select[name=isb_kewarganegaraan_i]').attr('readonly', true);
                        $('select[name=isb_kewarganegaraan_i]').val(data[i].KEWARGANEGARAAN_I);
                        $('input[name=isb_nik_i]').attr('readonly', true);
                        $('input[name=isb_nik_i]').val(data[i].NIK_I);
                        $('input[name=isb_nama_i]').attr('readonly', true);
                        $('input[name=isb_nama_i]').val(data[i].NAMA_I);
                        var objTtl = data[i].TTL_I;
                        var ttl = objTtl.split(' ');
                        $('input[name=isb_ttl_i]').attr('readonly', true);
                        $('input[name=isb_ttl_i]').val(ttl[0].substr(0, ttl[0].length - 1));
                        $('input[name=isb_ttl2_i]').attr('readonly', true);
                        $('input[name=isb_ttl2_i]').val(ttl[1]);
                        $('input[name=isb_umur_i]').attr('readonly', true);
                        $('input[name=isb_umur_i]').val(data[i].UMUR_I);
                        // $('select[name=isb_status_i]').attr('readonly', true);
                        // $('select[name=isb_status_i]').val(data[i].STATUS_I);
                        $('input[name=isb_agama_i]').attr('readonly', true);
                        $('input[name=isb_agama_i]').val(data[i].AGAMA_I);
                        $('input[name=isb_alamat_i]').attr('readonly', true);
                        $('input[name=isb_alamat_i]').val(data[i].ALMT_I);
                        $('input[name=isb_pekerjaan_i]').attr('readonly', true);
                        $('input[name=isb_pekerjaan_i]').val(data[i].PEKERJAAN_I).trigger('change');
                        $('input[name=isb_nohp_i]').attr('readonly', true);
                        $('input[name=isb_nohp_i]').val(data[i].NO_HP_I);
                    }
                },
                error: function() {
        ...
        //DELETE ISBAT
        $('#showDataIsbat').on('click', '.deleteDataIsbat', function() {
            var regId = $(this).attr('data');
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'ajax',
                        method: 'post',
                        url: '<?= BASE_URL . 'staff/deleteIsbat/'; ?>',
                        data: {
                            regId: regId
                        },
                        async: false,
                        ...
                        success: function(response) {
                            if (response.success) {
                                swal("Selamat!", "Data pasutri berhasil dihapus!", "success");
                                showDataIsbat();
                            } else {
        ...
        //SHOW DATA ISBAT
        function showDataIsbat() {
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= BASE_URL . 'staff/showDataIsbat'; ?>',
                async: false,
                dataType: 'json',
                success: function(data) {
                    var html = '';
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += '<td scope="col" style="width: 50%">' +
                            '<a href="javascript:;" class="btn round btn-sm btn-icon btn-info viewDetailIsbat" style="margin-left:5px;" data-toggle="tooltip" data-placement="bottom" title="LIhat Detail" data="' + data[i].REG_ID + '"><i class="ft-eye"></i></a>' +
                            '<a href="javascript:;" class="btn round btn-sm btn-icon btn-warning editDataIsbat" style="margin-left:5px;" data-toggle="tooltip" data-placement="bottom" title="Edit" data="' + data[i].REG_ID + '"><i class="ft-edit-2"></i></a>' +
                            '<a href="javascript:;" class="btn round btn-sm btn-icon btn-danger deleteDataIsbat" style="margin-left:5px;" data-toggle="tooltip" data-placement="bottom" title="Hapus" data="' + data[i].REG_ID + '"><i class="ft-x"></i></a>' +
                            '</td>' +
                            '</tr>';
                    }
                    $('#showDataIsbat').html(html);
